...
# For of the pymatgen.core.lattice module
...
class Lattice(MSONable):
    ...
    def find_all_mappings(
        ...
        inds = [
            np.logical_and(dist / l < 1 + ltol, dist / l >
                           1 / (1 + ltol))  # type: ignore
            for l in lengths
        ]
        ...
        for i, all_j in enumerate(gammab):
            ...
            for j, k in np.argwhere(inds):
                scale_m = np.array(
                    (f_a[i], f_b[j], f_c[k]), dtype=np.int)  # type: ignore
                ...
                if skip_rotation_matrix:
                    ...
                    rotation_m = np.linalg.solve(
                        aligned_m, other_lattice.matrix)
    ...
    def get_wigner_seitz_cell(self) -> List[List[np.ndarray]]:
        ...
        for r in tess.ridge_dict:
            if r[0] == 13 or r[1] == 13:
                to_return.append([tess.vertices[i]
                                  for i in tess.ridge_dict[r]])
    ...
    def get_points_in_sphere(
        ...
        try:
            # pylint: disable=C0415
            from pymatgen.optimization.neighborss import (
    ...
    def get_points_in_sphere_old(
        ...
        shifted_coords = fcoords[:, None, None,
                                 None, :] + images[None, :, :, :, :]
    ...
    def is_hexagonal(
        ...
        lengths = self.lengths
        angles = self.angles
        right_angles = [i for i in range(3) if abs(
            angles[i] - 90) < hex_angle_tol]
        hex_angles = [
            i
            for i in range(3)
    ...
    def get_distance_and_image(
        ...
        if jimage is None:
            ...
            fc = self.get_fractional_coords(
                v[0][0]) + frac_coords1 - frac_coords2
            fc = np.array(np.round(fc), dtype=np.int)
            return np.sqrt(d2[0, 0]), fc

        jimage = np.array(jimage)
        mapped_vec = self.get_cartesian_coords(
            jimage + frac_coords2 - frac_coords1)
...
def get_points_in_spheres(
    ...
    if np.any(pbc):
        ...
        # print("REC lattice", lattice.reciprocal_lattice)
        # print("REC_len", recp_len)
        maxr = np.ceil((r + 0.15) * recp_len / (2 * math.pi))
        # print("MAXREPEATS", maxr)
        ...
        all_ranges = [np.arange(x, y, dtype="int64")  # MISSING
                      for x, y in zip(nmin, nmax)]
        # print("ALL RANGES", np.array(all_ranges))
        matrix = lattice.matrix
        # print("MATRIX", matrix)
        # temporarily hold the fractional coordinates
        # print("All coords", all_coords)
        image_offsets = lattice.get_fractional_coords(
            all_coords) + numerical_tol
        # print("FRAC COORDS", image_offsets)
        all_fcoords = []
        ...
        # print("ALL FCOORDS", all_fcoords)
        image_offsets = image_offsets - all_fcoords
        # print("IMAGE offset ", image_offsets)
        coords_in_cell = np.dot(all_fcoords, matrix)
        # print("MAtrix", matrix)
        # print("COORDS in CELL ", coords_in_cell)
        ...
        for image in itertools.product(*all_ranges):
            # print("IMAGE", image)
            coords = np.dot(image, matrix)
            # print("COORS", coords)
            # print("CELL COORDS", coords_in_cell)
            ...
            # print("COMP MIN", coords>global_min[None, :])
            # print("COMP MAX", coords < global_max[None, :])
            # print(idx, "Valid index bool", valid_index_bool)
            ind = np.arange(len(all_coords))
            if np.any(valid_index_bool):
                #print("VALID", idx)
                #print("COORDS TO PUSH",coords[valid_index_bool] )
    ...
    # print("Valid", valid_coords)
    # print(valid_coords.shape)
    # Divide the valid 3D space into cubes and compute the cube ids
    # print("GLOBAL MIN", global_min, r)
    # print("PARAMETERS", valid_coords, global_min, r)
    all_cube_index = _compute_cube_index(valid_coords, global_min, r)
    # print("asdlfkjsadk", valid_coords, global_min, r)
    # print("ALL CUBE index", all_cube_index)
    nx, ny, nz = _compute_cube_index(global_max, global_min, r) + 1
    # print(nx,ny,nz)
    # print("ALL CUBE", all_cube_index)
    ...
    # print("SITE_CUBE_INDX", site_cube_index)
