...
class QuadraticMatrixPool:
    def __init__(self):
        self.matrix = {}

    def get(self, size):
        if size in self.matrix:
            return self.matrix[size]
        else:
            return np.zeros((size, size), dtype=np.float)

    def give(self, matrix):
        self.matrix[matrix.shape[0]] = matrix


quadratic_pool = QuadraticMatrixPool()
...
class EwaldSummation(MSONable):
    ...
    def __init__(
        ...
        self._eta = (
            eta if eta else (len(structure) * w /
                             (self._vol ** 2)) ** (1 / 3) * pi
        )
        ...
        self._gmax = (
            recip_space_cut if recip_space_cut else 2 * self._sqrt_eta * self._accf
        )
        ...
        self._oxi_states = [compute_average_oxidation_state(
    ...
    def compute_sub_structure(self, sub_structure, tol=1e-3):
        ...
        def find_match(site):
            for test_site in sub_structure:
                frac_diff = (
                    abs(np.array(site.frac_coords) -
                        np.array(test_site.frac_coords))
                    % 1
                )
                frac_diff = [abs(a) < tol or abs(
                    a) > 1 - tol for a in frac_diff]
                if all(frac_diff):
                    return test_site
            return None
    ...
    def get_site_energy(self, site_index):
        ...
        if self._charged:
            warn(
                "Per atom energies for charged structures not supported in EwaldSummation"
            )
        return (
            np.sum(self._recip[:, site_index])
            + np.sum(self._real[:, site_index])
    ...
    def _calc_recip(self):
        ...
        recip_nn = rcp_latt.get_points_in_sphere(
            [[0, 0, 0]], [0, 0, 0], self._gmax)

        frac_coords = [fcoords for (
            fcoords, dist, i, img) in recip_nn if dist != 0]
        ...
        for g, g2, gr, expval, sreal, simag in zip(
            gs, g2s, grs, expvals, sreals, simags
        ):
        ...
        for i in range(numsites):
            ...
            if self._compute_forces:
                ...
                fijpf = (
                    qj
                    / rij ** 3
                    * (erfcval + forcepf * rij * np.exp(-self._eta * rij ** 2))
                )
                forces[i] += np.sum(
                    np.frac_coords(fijpf, 1)
...
class EwaldMinimizer:
    ...
    def __init__(self, matrix, m_list, num_to_return=1, algo=ALGO_FAST):
        ...
        self._m_list = sorted(m_list, key=lambda x: comb(
            len(x[2]), x[1]), reverse=True)
        ...
        if algo == EwaldMinimizer.ALGO_COMPLETE:
            raise NotImplementedError(
                "Complete algo not yet implemented for " "EwaldMinimizer"
            )
    ...
    def minimize_matrix(self):
        ...
        """
        if (
            self._algo == EwaldMinimizer.ALGO_FAST
            or self._algo == EwaldMinimizer.ALGO_BEST_FIRST
        ):
    ...
    def add_m_list(self, matrix_sum, m_list):
        ...
        if (
            ...
            and len(self._output_lists) == self._num_to_return
        ):
            self._finished = True
    ...
    def best_case(self, matrix, m_list, indices_left):
        ...
        if self._algo == self.ALGO_TIME_LIMIT:
            ...
            interaction_correction = (
                average_correction * speedup_parameter
                + interaction_correction * (1 - speedup_parameter)
            )

        best_case = (
            np.sum(matrix)
            + np.inner(sums[::-1], fractions - 1)
            + interaction_correction
        )
...
def compute_average_oxidation_state(site):
    ...
    try:
        avg_oxi = sum(
            [sp.oxi_state * occu for sp, occu in site.species.items()
             if sp is not None]
        )
        return avg_oxi
